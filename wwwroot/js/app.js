...
async function startFogGame() {
    ...
    try {
        let seed = Math.floor(Math.random() * 9999999);
        const res = await fetch(`/api/template?zone=${currentZone}&template=${fogState.targetTemplate}&seed=${seed}`);
        currentMapData = await res.json();
...
async function startDrawGame() {
    ...
    try {
        const res = await fetch(`/api/template?zone=${zoneKey}&template=${targetTemplate}&seed=${seed}`);
        currentMapData = await res.json();
